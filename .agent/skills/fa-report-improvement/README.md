- ✅ **佈局強健性 (v2.1.5)**: 增加標題與內容佔位符的安全獲取機制，自動補救非標準版面
- ✅ **JSON 韌性 (v2.1.3)**: 自動清洗結尾符號、多餘逗號與 Markdown 標記
- ✅ 改善錯誤處理與日誌輸出
...
### 更新歷史
- **v2.1.5** (2026-01-29): 佈局容錯優化。解決當 PPT 佈局缺少標題或佔位符時會引發的 `AttributeError` 與 `IndexError`，增加 `get_or_create_title` 與 `get_or_create_body` 安全機制。
- **v2.1.4** (2026-01-28): UX 強化（支援檔案拖放上傳 Drag and Drop）
- **v2.1.3** (2026-01-28): 健壯性優化（JSON 自動容錯清洗、精確錯誤捕捉與前端預檢）
- **v2.1.2** (2026-01-28): 最終穩定版（包含 UX 優化與同步處理邏輯）
- **v2.1.1** (2026-01-28): 修正 Windows cp950 編碼問題 (UTF-8) 與路徑相容性
- **v2.1.0** (2026-01-28): 新增雙 JSON 格式支援
