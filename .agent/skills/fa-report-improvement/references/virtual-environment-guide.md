...
本專案推薦使用 `uv`，它是一個極速的 Python 套件與專案管理工具，能自動處理 Python 版本與虛擬環境。

### 為什麼選擇 uv?
- **速度極快**: 比 pip 快 10-100 倍。
- **自動管理**: 自動創建 `.venv`，自動安裝 Python。
- **跨平台**: Windows, Mac, Linux 指令一致。
- **鎖定依賴**: `uv.lock` 確保環境完全可重現。
